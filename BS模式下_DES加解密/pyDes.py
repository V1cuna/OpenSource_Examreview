...
class des(_baseDes):
    ...
    def __init__(self, key, mode=ECB, IV=None, pad=None, padmode=PAD_NORMAL):
        ...
        self.L = []
        self.R = []
        self.Kn = [[0] * 48] * 16  # 16轮 48-bit keys (K1 - K16)
        self.final = []
        self.setKey(key)
    ...
    def __String_to_BitList(self, data):
        """string to  (1, 0) stream"""
        if _pythonMajorVersion < 3:
            # strings to integers. Python 3 uses bytes
    ...
    def __des_crypt(self, block, crypt_type):
        ...
        self.L = block[:32]
        self.R = block[32:]
        # 从 Kn[1]  to Kn[16] 每轮用一个,加密16轮
        if crypt_type == des.ENCRYPT:
            #加密模式
            iteration = 0
            iteration_adjustment = 1
        # 解密倒过来,从Kn[16] to Kn[1]
        else:
            iteration = 15
            iteration_adjustment = -1
        i = 0
        while i < 16:
            ...
            # 扩展 or R[i - 1] with K[i], 生成 B[1] to B[8]
    ...
    def crypt(self, data, crypt_type):
        ...
        if not data:
            return ''
        if len(data) % self.block_size != 0:
            if crypt_type == des.DECRYPT:  # 8Byte msg
                raise ValueError(
